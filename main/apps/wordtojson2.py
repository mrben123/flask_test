...
def check_table_style1(index, block, content_list):
    key_title = "质量安全风险点清单"
    # return format_cell_text(table.cell(0, 0).text) == "乡镇(街道)" and \
    #        format_cell_text(table.cell(0, 1).text).endswith("业")
    return (key_title in content_list[index - 3].text) or (key_title in content_list[index - 2].text) or (key_title in content_list[index - 1].text)


def check_table_style2(index, block, content_list):
    key_title = "乡镇监管机构负责人信息表"
    return (key_title in content_list[index - 3].text) or (key_title in content_list[index - 2].text) or (key_title in content_list[index - 1].text)

def check_table_style3(index, block, content_list):
    key_title = "安全工作负责人信息表"
    return (key_title in content_list[index - 3].text) or (key_title in content_list[index - 2].text) or (key_title in content_list[index - 1].text)
...
def get_city(index, content_list):
    i = 0
    while i < 3:
        i = i + 1
        if (index - i) < 0:
            continue
        temp = format_cell_text(content_list[index - i].text)
        if ("市" in temp) or ("区" in temp) or ("县" in temp):
            return temp
    ...
    try:
        ...
        for index, block in enumerate(content_list):
            if "Table" in block.style.name:
                if check_table_style1(index, block, content_list):
                    city_and_area = get_city(index, content_list)
        ...
        if(len(data_list1)>0):
            ...
            output["attachment_1"] = file_name + "_1.json"
        if(len(data_list2)>0):
            write_list_to_json(data_list2, path_dir + file_name + "_2.json")
            output["attachment_2"] = file_name + "_2.json"
        if(len(data_list3)>0):
            write_list_to_json(data_list3, path_dir + file_name + "_3.json")
            output["attachment_3"] = file_name + "_3.json"
        str2 = "json=" + json.dumps(output, ensure_ascii=False)
        print(str2)
